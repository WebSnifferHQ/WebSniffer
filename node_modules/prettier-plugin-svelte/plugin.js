...
const options = {
    ...
    svelteWhatToFormat: {
        ...
        choices: [
            ...
            makeChoice('markup'),
            makeChoice('styles'),
            makeChoice('scripts-markup'),
            makeChoice('scripts-styles'),
            makeChoice('markup-styles'),
        ],
        default: 'all',
        description: 'What to format in Svelte files',
    },
};
...
function shouldFormat(part, whatToFormat) {
    return whatToFormat === 'all' || whatToFormat.includes(part);
}
...
function embed(path, print, textToDoc, options) {
    ...
    if (node.isJS) {
        if (!shouldFormat('script', options.svelteWhatToFormat)) {
    ...
    const embedType = (tag, parser, isTopLevel) => {
        if (!shouldFormat(tag === 'template' ? 'markup' : tag, options.svelteWhatToFormat)) {
            return getText(node, options, true);
        }
...
function embedTag(tag, text, path, formatBodyContent, print, isTopLevel, options) {
    ...
    if (isTopLevel && options.svelteSortOrder !== 'none') {
        const after = shouldFormat('markup', options.svelteWhatToFormat) ? hardline$1 : '';
        ...
        // the new position.
        if (previousComment) {
            result = concat$1(['<!--', previousComment.data, '-->', hardline$1, result, after]);
        }
        else {
            result = concat$1([result, after]);
        }
    }
    return result;
}
